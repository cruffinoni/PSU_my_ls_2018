...
static int add_subfolder(t_ls_flags flags, t_file *node, char *path)
{
    char *ptr = NULL;
    ...
    if ((flags & FLAG_R) && node->dirent->d_type == DT_DIR &&
        ...
        if (path == NULL)
            return (ERR_MALLOC);
        add_folder(&(node->subf), path, ptr, flags);
        free(ptr);
    }
    free(path);
    return (ERR_NONE);
...
static int add_file(t_folder *folder, t_dirent *dirent, t_ls_flags flags,
    char *path)
{
